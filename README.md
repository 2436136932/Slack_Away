...
- **5 个游戏**：
  ...
  - **红中麻将**：合肥规则 112 张（万筒条×4 + 红中×4），红中=赖子百搭，无吃只碰杠，**只准自摸胡（杠上开花算自摸）**，3 个本地 AI + 大模型出牌双模式，胡牌判定带赖子回溯拆解（15 项单测）；**碰/杠 toast 动画 + 我的副露区 + 胡牌定格面板 + 听牌提示 + 玩家暗杠/补杠 + 出牌飞行动画 + 碰杠 6s 倒计时自动过 + 手牌花色分区 + 一圈四局轮庄 + 本地对局统计 + 完整计番与积分结算 + 四方桌布局 + AI 防喂牌**
