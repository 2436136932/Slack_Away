...
> 🧊 桌面透明玻璃摸鱼小游戏集 —— Apple 毛玻璃悬浮卡片 · **五子棋 / 象棋 / 连连看 / 2048 / 扫雷 / 红中麻将** · 本地 AI + 大模型双模式 · Alt+G 一键隐身
...
- **5 个游戏**：
  - 五子棋 / 象棋：Canvas 绘制，棋子/棋盘/选中/上一步痕迹全部走"低对比 + 随玻璃透明度向中性灰收敛"公式，永远不抢眼；**空心棋子**（底座全透明，棋盘线穿透显示）
  - **连连看**：BFS 寻路（≤2 转折），路径 SVG 折线高亮，无解自动重排剩余方块
  - **2048**：鼠标拖拽 / 方向键 / WASD 三种输入，灰度梯度配色
  - **扫雷**：左挖右旗，首挖必安全（首点+8 邻域不布雷），洪水扩散，数字低饱和配色
  - **红中麻将**：合肥规则 112 张（万筒条×4 + 红中×4），红中=赖子百搭，无吃只碰杠，**只准自摸胡（杠上开花算自摸）**，3 个本地 AI + 大模型出牌双模式，胡牌判定带赖子回溯拆解（15 项单测）；**碰/杠 toast 动画 + 我的副露区 + 胡牌定格面板 + 听牌提示 + 玩家暗杠/补杠 + 出牌飞行动画 + 碰杠 6s 倒计时自动过 + 手牌花色分区 + 一圈四局轮庄 + 本地对局统计**
- **悔棋**：撤销最近一步（vs-AI 棋类撤玩家+AI 各一步；单人游戏撤单步）
- **三档难度**：每个游戏都映射（棋类走 AI 深度，单人游戏走棋盘大小 / 雷数）
- **本地 AI + 大模型双模式**（仅五子棋 / 象棋，无 AI 游戏自动隐藏此控件）：
  - 本地：五子棋棋型评估 / 象棋 negamax 搜索（复用原项目 `xiangqi.js` 引擎）
  - 大模型：任意 OpenAI 兼容接口（DeepSeek / New API / Kimi…），一键拉取模型列表下拉选择，走棋失败自动回退本地
...
- `09-模型下拉面板-全量列表.png`（LLM 配置：自绘下拉面板，自动列出接口模型）
- `10-连连看-消一对.png`（6×8 棋盘，emoji 清淡融入玻璃）
- `11-2048-合并后.png`（4×4 滑动四次，灰度梯度区分 2/4）
- `12-扫雷-首挖展开.png`（12×12 首挖中间自动扩散 60 格，低饱和数字配色）
- `13-红中麻将-对局中.png`（6 游戏顶栏 + 三对手区 + 手牌花色低饱和 + 模式控件自动隐藏）
- `14-红中麻将-胡牌定格.png`（胡牌定格面板：胡牌/手牌/副露/番型全摊开）
