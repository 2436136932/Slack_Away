...
> 🧊 桌面透明玻璃摸鱼小游戏集 —— Apple 毛玻璃悬浮卡片 · **五子棋 / 象棋 / 连连看 / 2048 / 扫雷 / 红中麻将 / 斗兽棋 / 台球** · 本地 AI + 大模型双模式 · Alt+G 一键隐身
...
- **5 个游戏**：
  ...
  - **台球**：中式八球人机对战，手写物理引擎（子步进碰撞防穿模），5 种辅助线（瞄准线/进球预测/库边反弹/力度条/球组高亮），AI 三难度
  - **斗兽棋**：8×8 动物棋，象狮虎豹狼狗猫鼠等级吃子，鼠吃象、狮虎跳河、陷阱变弱、占兽穴即胜，本地 AI 三难度
  - **红中麻将**：合肥规则 112 张（万筒条×4 + 红中×4），红中=赖子百搭，无吃只碰杠，**只准自摸胡（杠上开花算自摸）**，3 个本地 AI + 大模型出牌双模式，胡牌判定带赖子回溯拆解（15 项单测）；**碰/杠 toast 动画 + 我的副露区 + 胡牌定格面板 + 听牌提示 + 玩家暗杠/补杠 + 出牌飞行动画 + 手牌花色分区 + 一圈四局轮庄 + 本地对局统计 + 完整计番（含七对/龙七对）与积分结算 + 四方桌布局 + AI 防喂牌 + 自摸手动点胡 + 一圈战绩结算面板 + 记牌器（各家已出牌余量） + 小屏自适应 + 牌面重绘（万/条/筒汉字+三色底色）+ 听牌引导高亮**
...
- **汉堡菜单**：6 个游戏 + 难度/模式/LLM/透明度/墨色/幽灵全部收进左侧滑出抽屉，界面更省空间（悔棋/新局常驻）
...
- `15-汉堡菜单.png`（左侧滑出抽屉：游戏列表 + 全部设置）
- `16-红中麻将-四方桌.png`
- `17-斗兽棋.png`
- `18-台球.png`（中式八球对战 + 5 种辅助线）（8×8 动物棋，16 子初始布局 + 陷阱/兽穴/河区）（四方桌布局：对手围三方 + 牌河四分区 + 头像/昵称/牌背朝向/落位微弹）
